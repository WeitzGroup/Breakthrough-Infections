...
# filter df to phi=0 for interpolation
df_phi0 <- df %>% filter(phi == 0)
...
#plot figure 1
pdf("../../output/figures/fig1.pdf", height=6, width=12)
# panel A
p1a <- ggplot()+
...
print(plot_grid(p1a, p1b, labels="AUTO", label_size=20, hjust=-1, label_x=-.03))
dev.off()

# Figure 2
pdf("../../output/figures/fig2.pdf", height=6, width=6)
...
p4a <- df %>%
  ...
  # customize legend: nrow=1 keeps all phi values on one row
...
print(plot_grid(p4a+theme(legend.position="none"), 
          p4b+theme(legend.position="none"), 
          nrow=1,
          labels="AUTO") %>%
  plot_grid(., leg, nrow=2, rel_heights = c(8, 1.5), label_size=20, hjust=-1, label_x=-.03))
dev.off()
