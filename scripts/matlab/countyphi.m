...
%% Generates: countyphi-ode-output.csv 
...
% Range of vaccine coverage values (hard-coded from county-level values)
P_main = [.76, .86, .95, .83, .91, .82, .90];
...
for ii = 1:length(P_main)*length(PHI)
    ...
    if mod(ii,100) == 0
        ...
        writetable(SteadyStates_main, fullfile(output_dir, 'countyphi-ode-output.csv'));
...
% Main output file
writetable(SteadyStates_main, fullfile(output_dir, 'countyphi-ode-output.csv'));
fprintf('Saved: %s/countyphi-ode-output.csv\n', output_dir);

% Timestamped backups
timestamp = string(datetime('now'));
timestamp = strrep(timestamp,' ','_');
timestamp = strrep(timestamp,':','-');
backup_csv = sprintf("countyphi_%s.csv", timestamp);
writetable(SteadyStates_main, backup_csv);
backup_mat = sprintf("countyphi_%s.mat", timestamp);
save(backup_mat);
