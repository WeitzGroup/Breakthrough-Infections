%% This code generates the main ODE simulation output for breakthrough infections analysis
...
if isfile(phi_file)
    ...
    % PHI includes: 0 (baseline), estimated values from school data, and high values for comparison
...
writetable(SteadyStates, fullfile(output_dir, 'ode_out.csv'));
fprintf('Saved: %s/ode_out.csv\n', output_dir);
...
backup_csv = sprintf("main_%s.csv", timestamp);
writetable(SteadyStates, backup_csv);
backup_mat = sprintf("main_%s.mat", timestamp);
save(backup_mat);
