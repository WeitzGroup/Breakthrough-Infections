%% This code generates Figure S4 (Across different reproduction numbers (R0) and primary
%% failure rates (ε), breakthrough infections (IV ) peak at intermediate
%% vaccine coverage (p)Breakthrough infections peak at intermediate vaccine coverage) of the manuscript
...
filename = sprintf("FigureS4_%s.csv",timestamp);
writetable(SteadyStates,filename);
filename = sprintf("FigureS4_%s.mat",timestamp);
save(filename);
